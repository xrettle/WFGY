...
讓你不用先下載 ZIP，也能先看懂這批實驗在驗證什麼、為什麼跟 AI 成本有關、四個證據包各自代表什麼，以及目前結果能說什麼、不能說什麼。

ZIP 檔主要是給想重現、研究、審核、檢查原始輸出的人使用。
...
| 四包分別是什麼 | [四個公開證據包](#evidence-packs) |
...
> PP02A 測版本演化與證據路徑。
>
> PP02B 測偏數學任務中的符號、條件、反例與推理壓力。
>
> PP02C 測程式修復、來源邊界、沙盒檢查與硬性紅線。
>
...
目前 downloads 資料夾中可能保留部分早期過程檔案。第一次閱讀請以上面四個正式公開證據包為準。
...
| 可以說 | 說明 |
| --- | --- |
| Polaris 在公開任務範圍內顯示出降低 token 成本的強訊號 | 尤其是 PP01 主成本實驗 |
| 結構化任務表示有機會保留可檢查輸出品質 | 不是只看答案好不好看，也看證據鏈 |
| 四個公開包覆蓋不同風險面 | 成本、版本演化、偏數學、程式修復與紅線 |
