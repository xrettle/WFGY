...
* There is no widely accepted, unified mathematical framework that:
...
* it constrains what is possible in self-improving and energy-limited AI systems, and
...
3. A staging ground for an explicit MVP:
   ...
   * Q130 is designed such that a minimal Colab implementation can demonstrate:
     ...
     * how a simple OOD tension score separates grounded from ungrounded answers, and
...
5. R. Geirhos, J.-H. Jacobsen, C. Michaelis, R. Zemel, W. Brendel, M. Bethge, F. A. Wichmann,
   ...
   Nature Machine Intelligence, volume 2, pages 665–673, 2020.
...
These weights are part of the encoding and are chosen once before running any experiments. They are not tuned per dataset or per model.
...
To avoid hidden tuning, we impose explicit constraints on what counts as an admissible encoding.
...
* The weights a_ref, a_ground, and a_out are chosen once, based on:
  ...
  * a small amount of calibration that does not include test tasks.

* After they are chosen, they remain fixed for all experiments involving Q130.
...
* For each integer k, M_k is defined by an explicit complexity limit, such as:
  ...
  * maximum relation count less than or equal to fk for some fixed function f.

* All comparisons of tension across refinement levels are made using the nested sequence M_1 subset M_2 subset M_3 and so on.
...
These rules ensure that Tension_OOD is not a hidden post hoc tuning device but a stable observable.
...
Evaluate whether Tension_OOD(m) aligns with actual common-sense failures on a static benchmark of physical reasoning tasks under distribution shift.
...
* For each scenario and model under test, construct a state m in M_k that includes:
...
* Stability of these relationships when changing model architectures within the same encoding.

*Falsification conditions:*

* If Tension_OOD(m) shows no statistically meaningful correlation with error rates across slices and tasks, the current encoding of DeltaS_ref, DeltaS_ground, DeltaS_outcome, or the weights a_ref, a_ground, a_out is rejected.
* If small, global changes in the encoding parameters (still respecting the admissible class) lead to arbitrarily different correlations with no stable pattern, the encoding is considered unstable and rejected.
...
Falsifying TU encoding != solving canonical statement. This experiment can only reject particular encodings of Tension_OOD; it cannot prove that robust OOD generalization is impossible or fully characterize it when it succeeds.
...
* If Tension_OOD distributions for the grounded and ungrounded worlds are not meaningfully separated, the encoding fails to capture the intended grounding differences and is rejected.
* If an encoding assigns consistently lower Tension_OOD to the ungrounded world than to the grounded world, it is considered inverted and rejected.
...
Falsifying TU encoding != solving canonical statement. This experiment only tests whether Q130 encoding can recognize simple synthetic grounded vs ungrounded systems; it does not fully characterize real AI systems.
...
Falsifying TU encoding != solving canonical statement. A failed MVP only shows that the current observable definitions or guidance strategy are insufficient; it does not rule out better encodings within the Q130 framework.
...
* E_level: E2
  ...
  * A full effective layer encoding has been given:
    ...
    * Tension_OOD(m) and its admissible encoding class,
    * explicit singular set S_sing and domain restriction.

  * Experiments that can falsify specific encodings have been specified, including an MVP suitable for a small notebook.
