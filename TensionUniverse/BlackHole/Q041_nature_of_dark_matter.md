...
In standard cosmology, several lines of evidence indicate the presence of non luminous, non baryonic matter that interacts primarily through gravity. These include:

* galaxy rotation curves that remain approximately flat at large radii rather than declining,
...
The dark matter problem, in its canonical form, is not just to fit any additional mass component to data, but to identify a description that:
...
3. remains stable under improved measurements and more precise modelling.

There are two broad classes of approaches at the effective level:
...
* modifications to gravity or inertia, sometimes in combination with reduced dark matter sectors.

Q041 treats this not as a search for a specific particle or gravity law, but as a question about whether a low tension description of the unseen mass sector exists in a constrained encoding class.

### 1.2 Status and difficulty

Empirically, the cold dark matter plus cosmological constant model provides a good first order fit to many observations. However:

* small scale structure shows potential tensions with simple cold collisionless dark matter descriptions, including issues such as cores versus cusps in galaxy centers and the abundance of small satellites,
* the detailed properties of dark matter, including its mass, interactions, and possible subcomponents, remain unknown,
* alternative frameworks, including modified gravity or hybrid scenarios, can fit some subsets of data but often struggle to match the full multi channel evidence simultaneously.

No single candidate or framework has been confirmed. Direct detection experiments, collider searches, and astrophysical probes have so far yielded null or ambiguous results. The problem is therefore open and remains central to cosmology and particle astrophysics.

Within the BlackHole project, the difficulty arises from the need to unify:

* high precision cosmological measurements,
* complex astrophysical environments with baryonic feedback,
* and an unseen sector that must be inferred indirectly through consistency across many observables.
...
3. A source of reusable components for:
   ...
   * more general reasoning about hidden mechanisms in other domains.
...
4. NASA or ESA cosmology overview page on “Dark Matter”.
   Provides an institutional summary of the evidence for dark matter, typical parameter values, and current experimental searches.

---

## 2. Position in the BlackHole graph

This block records the planned position of Q041 in the Q001–Q125 graph, in terms of upstream, downstream, parallel, and cross domain edges. Each edge has a single line reason referencing concrete components or tension types.
...
* Q021 (BH_PHYS_QG_L3_021)
  Reason: Provides the gravitational sector background and constraints on allowed large scale gravitational behaviour, which Q041 uses when defining admissible encodings for cosmic fields.

* Q025 (BH_PHYS_BARYON_ASYM_L3_025)
  Reason: Fixes the allowed baryon budget and asymmetry, which Q041 reuses when defining baryon fraction tension.
...
* Q043 (BH_COSMO_INFLATION_L3_043)
  Reason: Provides the primordial perturbation spectra and refine(k) idea for multi scale power spectra that Q041 uses in its spectral mismatch.

### 2.2 Downstream problems

These problems reuse Q041 components or treat Q041 as a prerequisite.

* Q042 (BH_COSMO_DARKENERGY_L3_042)
  Reason: Reuses Q041’s MultiChannel_Cosmo_Descriptor and energy budget consistency to define tension between dark matter, dark energy, and geometry.
...
* Q036 (BH_PHYS_HIGH_TC_MECH_L3_036)
  Reason: Both deal with hidden microscopic sectors inferred from macroscopic evidence under a consistency_tension pattern.
...
These edges connect Q041 to problems in other domains that reuse its components.

* Q059 (BH_CS_INFO_THERMODYN_L3_059)
  Reason: Reuses Q041’s treatment of energy and information budgets to structure information thermodynamics in computational settings.

* Q091 (BH_EARTH_CLIMATE_SENS_L3_091)
  Reason: Adopts Q041’s parameter sensitivity and multi channel evidence templates for climate sensitivity and feedback analysis.

* Q121 (BH_AI_ALIGNMENT_L3_121)
  Reason: Mirrors Q041’s pattern of constraining a hidden mechanism using multiple observational channels when reasoning about aligned agents.

* Q123 (BH_AI_INTERP_L3_123)
  Reason: Reuses the idea of inferring latent structure from many coarse projections, with Q041 as the cosmological prototype.

---

## 3. Tension Universe encoding (effective layer)

All content in this block stays at the effective layer. We only specify:
...
We do not describe any deep level generative rule, any mapping from raw data to TU fields, or any constructive procedure for the internal cosmological fields.
...
* these states respect basic physical constraints such as positivity of densities and consistent matter content.
...
* At the effective layer, this is defined as the part of `rho_tot` not accounted for by known baryons and radiation, after a chosen encoding procedure.
...
We do not specify the internal structure of `obs_channel_i(m)`; we only require that they are finite dimensional and consistent with basic physical constraints.
...
* A reference baryon fraction profile:
  ...
  which can be taken as a constant or a mild function of scale within an allowed range based on nucleosynthesis and CMB constraints.
...
with weights satisfying the fairness constraints:
...
* `Tension_DM(m)` is small when the matter power spectrum, multi channel observables, and baryon fraction all lie close to their reference expectations within the encoding class.
...
1. A finite library of dark matter model families which generate reference profiles for `ref_P_CDM` and `ref_f_baryon`. Examples include:
   ...
   * self interacting dark matter models within bounded cross section ranges,
   * limited hybrid scenarios where a dark matter component coexists with modified dynamics.

2. A rule for choosing `ref_P_CDM` and `ref_f_baryon` from this library that does not depend on the specific data to be tested. For instance:

   * choose a small set of canonical parameter points that are representative of the library,
   * define reference profiles as averages or envelopes over this set.
...
* The admissible class must be constrained enough that different choices within it do not arbitrarily drive `Tension_DM(m)` to zero for any possible configuration, otherwise the encoding is judged too flexible.

These constraints are designed to prevent the dark matter tension functional from being reduced by retrospective parameter tuning.

### 3.6 Effective tension tensor components

In line with the general TU core, we introduce an effective tension tensor:
...
* `lambda(m)` indicates the local convergence state of reasoning or modelling, in a fixed bounded range.
...
S_sing_DM = {
  m in M_DM :
    ...
    or the admissible encoding rules cannot be applied
      (for example missing essential meta information)
}
```
...
* All definitions of `DeltaS_spectrum`, `DeltaS_channels`, `DeltaS_baryon`, `Tension_DM`, and `T_ij` are only considered meaningful on `M_DM_reg`.
...
This block states how Q041 is expressed as a tension principle within TU.
...
* In a low tension world, one can find regular states `m` in `M_DM_reg` that represent the real universe and satisfy:
  ...
  for a small threshold `epsilon_DM` that does not blow up as refine(k) increases resolution or data quality.
...
Here refine(k) refers to moving along a predetermined scale and data quality sequence indexed by an integer `k`, for example increasing the range of `k_bin` in the power spectrum and adding more precise channel summaries.
...
This is an effective way to say that the admissible dark matter sector can keep all observables mutually consistent as we gain more information.

### 4.3 Persistent high tension and failure modes

If no genuine dark matter sector within the admissible class can explain observations, or if modified gravity or radical alternatives fail to reproduce all channels, we expect at least one of the following persistent high tension patterns:

1. Spectral failure

   * Even after parameter exploration within the admissible library, `DeltaS_spectrum(m_k)` stays above a positive lower bound that cannot be justified by known uncertainties.
...
for some `delta_DM > 0` that does not shrink as k grows. Q041 uses these patterns to classify scenarios rather than to pick a specific microphysical theory.

---

## 5. Counterfactual tension worlds

We now describe counterfactual worlds at the effective layer. These worlds are patterns in the observables and tension functionals, not deep level constructions.

### 5.1 World T_DM: genuine dark matter sector, low tension

In World T_DM:

1. There exists a dark matter sector within the admissible library such that for states `m_T,k` along a refine(k) sequence:

   ```txt
   Tension_DM(m_T,k) stays within Band_low
   ```

   even as the resolution and data quality improve.

2. Spectral consistency

   * `DeltaS_spectrum(m_T,k)` decreases or stabilises as more scales are included, remaining compatible with a cold or closely related dark matter profile.

3. Channel consistency

   * Channel summaries from rotation curves, lensing, clusters, and CMB all fit within a single dark matter plus baryon configuration, so `DeltaS_channels(m_T,k)` remains small.
...
This world does not specify the microscopic nature of dark matter; it states that a coherent dark sector mechanism exists whose effective behaviour keeps tension low.

### 5.2 World F_MOD: purely modified gravity or radical alternative, high tension

In World F_MOD:

1. The true universe has no separate dark matter sector. Instead, modifications to gravity or inertia attempt to account for observations.

2. When these alternatives are encoded within the same admissible framework, any attempt to interpret data through such models leads to one or more of the following:

   * `DeltaS_spectrum(m_F,k)` stays high because matter clustering does not match the reference band in a way compatible with baryon and radiation content,
   * `DeltaS_channels(m_F,k)` remains high since a single set of modified dynamics cannot fit all channels simultaneously,
...
Thus, for all regular states representing the universe at high enough k:
...
1. A combination of dark matter and modified dynamics is allowed, possibly with additional fields or coupling rules.

2. If the hybrid approach genuinely reduces tension in a non fine tuned way, we expect a pattern intermediate between World T_DM and World F_MOD:

   * some components of `Tension_DM` decrease due to added flexibility,
   * others may increase if additional complexity introduces new inconsistencies.

3. Q041 treats World MIX as a test of whether more complex explanations provide real gains in tension reduction or simply redistribute inconsistencies.

In the encoding, World MIX is evaluated by comparing tension profiles to those in World T_DM and World F_MOD, without favouring any particular microphysical interpretation.

---

## 6. Falsifiability and discriminating experiments

This block describes experiments and protocols that can falsify or support specific Q041 encodings at the effective layer. They do not prove or disprove the existence of dark matter in an absolute sense; they test whether a given encoding class and tension functional is adequate and non trivial.

### Experiment 1: Joint cosmological and astrophysical tension survey

*Goal:*

Evaluate whether a single dark matter encoding from the admissible library can keep `Tension_DM` within a controlled band across all major observational channels.
...
* Inputs:
  ...
  * summary data for:
    ...
    * cluster dynamics,
    * CMB anisotropies relevant to matter content,
  ...
  * fixed weights `(w_spec, w_chan, w_baryon)` with each strictly positive and summing to 1.
...
3. For each data combination, record:
   ...
   * the minimal `Tension_DM` achieved within the library,
   * the distribution of tension values over models.

4. Repeat the procedure for increasing refine(k) steps, for example by:
...
* If, for all models in the admissible library and for sufficiently large k, minimal `Tension_DM` exceeds a conservative upper bound `epsilon_max` in at least one major channel combination, then the current encoding class is considered falsified as a low tension description.
* If small adjustments within the admissible library and weight constraints produce arbitrarily different tension profiles without clear physical justification, the encoding is judged too flexible and rejected.

*Semantics implementation note:*

All fields and mismatch quantities are treated in a hybrid representation that combines continuous matter fields and discrete scale bins. No additional semantic type is introduced beyond this, and no dependence on token level representations occurs.

*Boundary note:*

Falsifying TU encoding != solving canonical statement. This experiment can reject the specific dark matter tension encoding and admissible class; it does not prove or disprove the existence of dark matter itself.
...
2. For both Group T and Group F, compile the distributions of `Tension_DM`.

3. Study the separation between the tension distributions as refine(k) increases:

   * for example, by introducing more realistic small scale physics or improved synthetic observation noise models.
...
Simulated universes and their summaries are treated using the same hybrid representation as the real data case. The experiment does not rely on or introduce any additional encoding type beyond the one fixed in Block 0.

*Boundary note:*

Falsifying TU encoding != solving canonical statement. Success or failure on mock universes tests the usefulness of the encoding; it does not directly settle the question of dark matter in our universe.

---

## 7. AI and WFGY engineering spec

This block describes how Q041 can be used as an engineering module for AI systems within WFGY, still at the effective layer.

### 7.1 Training signals

We define several training signals derived from Q041 observables.
...
1. `signal_DM_consistency`
   ...
   * Definition:
     ...
     used as a penalty that encourages internal states with low dark matter tension in contexts where a standard dark matter interpretation is assumed.
...
2. `signal_channel_conflict`
   ...
   * Definition:
     ...
     used to penalise internal states where explanations for different observational channels imply inconsistent matter configurations.
...
3. `signal_baryon_budget`
   ...
   * Definition:
     ...
     enforces awareness that baryons cannot account for all inferred mass while respecting nucleosynthesis and CMB constraints.

4. `signal_world_switch_stability_DM`

   * Definition:

     * a signal derived by comparing model outputs when prompted under:

       * “assume a standard dark matter sector”,
       * “assume no dark matter and attempt to fit data otherwise”.

     * It measures how cleanly the model separates these regimes and how consistently it applies each assumption.

### 7.2 Architectural patterns

We outline architectural patterns that reuse Q041 structures.
...
1. `CosmoTensionHead_DM`
   ...
   * Role:
     ...
     * a head that reads internal representations of cosmological questions and outputs an estimate of:
       ...
       * `Tension_DM`,
       * potentially a vector summary of `DeltaS_spectrum`, `DeltaS_channels`, and `DeltaS_baryon`.

   * Interface:

     ```txt
     Input: internal embedding representing a cosmology context
     Output: scalar tension estimate and small vector of mismatch scores
     ```
...
2. `EvidenceChannelAggregator_DM`
   ...
   * Interface:
     ...
     ```txt
     Input: sequence of channel specific embeddings
     Output: MultiChannel_Cosmo_Descriptor style feature vector
     ```
...
3. `TU_CosmoObserver_DM`
   ...
   * Interface:
     ...
     ```txt
     Input: internal representation of a cosmological scenario
     Output: structured summary suitable for feeding into CosmoTensionHead_DM
     ```
...
1. Task selection
   ...
   * Build a set of cosmology questions that involve:
     ...
     * comparisons between dark matter and modified gravity,
...
4. Metrics
   ...
   * Structural quality of explanations, for example:
     ...
     * consistent treatment of baryon budgets.

   * Consistency between answers across different prompts that change assumptions about dark matter presence.

### 7.4 60 second reproduction protocol

This provides a minimal protocol for external users to experience the effect of Q041 style encoding in an AI system.
...
* TU encoded setup
  ...
  * Prompt:
    ...
    * ask the same question, but instruct the model to organise the answer around:
      ...
      * the idea that a single dark sector must explain all channels consistently.
  ...
  * Observation:
    ...
    * record whether the answer:
      ...
      * clearly notes what would go wrong in a world without dark matter.
...
* What to log
  ...
  * Prompts, full answers, and any auxiliary tension scores emitted by CosmoTensionHead_DM, to allow later auditing of effective layer behaviour.

---

## 8. Cross problem transfer template

This block describes reusable components produced by Q041 and their direct reuse targets.
...
1. Q042 (Nature of dark energy)
   ...
   * What changes:
     ...
     * the functional focuses more on geometry and expansion observables, but keeps the same basic interface.
...
3. Q050 (Testability of multiverse scenarios)
   ...
   * What changes:
     ...
     * the model_class parameter in the template spans broader sets of cosmological models than for dark matter alone.
...
4. Q121 (AI alignment problem)
   ...
   * What changes:
     ...
     * observables are now behavioural or decision channels rather than astrophysical data.

---

## 9. TU roadmap and verification levels

This block records the current verification levels for Q041 and the next measurable steps.
...
* N_level: N1
  ...
  * The graph placement is defined in terms of upstream, downstream, parallel, and cross domain edges.

### 9.2 Next measurable step toward E2

To move Q041 from E1 to E2, at least one of the following should be implemented:
...
1. A concrete numerical pipeline that:
   ...
   * publishes tension profiles for a small admissible library of dark matter models.
...
2. A suite of mock universe experiments where:
   ...
   * results and code are made reproducible for independent groups.
...
* a benchmark for testing whether TU style tension functionals can remain informative and non trivial even for highly contested open problems.
...
This unseen mass is called dark matter. The hard part is not just adding some invisible mass to equations. The hard part is finding a single description of this dark matter that works for all the different kinds of observations at once.
...
* Build a way to measure how “tense” they are with each other if you assume a certain kind of dark matter.
...
* simple numbers that measure how far those states are from a standard dark matter picture:
  ...
  * one number for whether the amount of normal matter stays in the allowed range,

* a combined tension score that is small when everything fits together and large when something is wrong.

Then it compares different possible worlds:

* In a world where dark matter is real and well described by some model in our library, there should be a way to keep the combined tension score low across all observations, even as they improve.
* In a world with no dark matter, or with only modified gravity, the combined tension score should eventually become large and hard to reduce without special tricks.

This does not prove what dark matter is. It does something more modest and precise:

* it turns the vague idea “all the evidence fits” into a measurable statement about tension between channels,
* it offers experiments that can show whether a given way of encoding dark matter is too flexible or too rigid,
* it provides reusable tools that can be applied to other hidden sector problems, in physics and beyond.

Q041 is therefore the central node for dark matter in the Tension Universe framework, and a model for how to handle big open questions that depend on unseen parts of the universe.
